...
void navigate_to_waypoint(float x, float y, float ang_d)
{
	...
	rotate_degs = normalize_angle_value(ang_d);
	writeDebugStream("Rotate angle: %f\n",ang_d);

	// Rotate towards the correct position
	rotate(ang_d);
	points_update(ang_d, ROTATE_STATE);
...
//	rotate(90);
	...
	print_10_points();
	navigate_to_waypoint(50,50,45);
	print_10_points();
	navigate_to_waypoint(50,-20,-130);
	print_10_points();
	navigate_to_waypoint(0,0,-110);
	print_10_points();
